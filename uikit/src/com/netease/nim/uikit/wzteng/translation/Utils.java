...
public class Utils {
     ...
     * @return 错误信息
     */
    public static String getErrorMessage(TranslationResult result) {
        if (result == null)
            return "没有结果显示";

        if (result.getErrorCode() == Translator.ERROR_CODE_NONE)//正常
            return null;

        String error;
        switch (result.getErrorCode()) {
            case Translator.ERROR_CODE_RESTRICTED:
                error = "请求过于频繁，请尝试更换API KEY";
                break;
            case Translator.ERROR_CODE_INVALID_KEY:
                error = "无效的API KEY";
                break;
            case Translator.ERROR_CODE_QUERY_TOO_LONG:
                error = "要翻译的文本过长";
                break;
            case Translator.ERROR_CODE_UNSUPPORTED_LANG:
                error = "不支持的语言类型";
                break;
            case Translator.ERROR_CODE_NO_RESULT:
            default:
                error = "没有结果显示";
                break;
        }
     ...
     */
    public static String[] splitExplain(String input) {
